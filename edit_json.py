...
def resize_coordinates(x, y, original_shape, new_shape):
    """Rescale coordinates from original image size to new image size."""
    scale_x = new_shape[1] / original_shape[1]
    scale_y = new_shape[0] / original_shape[0]
    return round(x * scale_x, 2), round(y * scale_y, 2)

def update_json_coordinates(json_path, new_shape=(512, 512)):
    ...
    if "shapes" in data:
        for shape in data["shapes"]:
            if "points" in shape:
                shape["points"] = [resize_coordinates(x, y, original_shape, new_shape)
